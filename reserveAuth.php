...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $insertQuery = "INSERT INTO reservations (productid, date, name, category, sku, description, image, status, userID, userName, schoolID)
                        SELECT id, CURRENT_DATE(), ?, category, sku, description, image, ?, ?, ?, ?
                        FROM inventory
                        WHERE id = ?";
